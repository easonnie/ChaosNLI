...
def main():
    ...
    parser.add_argument("--eval_dataset_name", type=str, default=None, help='the name tag for the training dataset')
...
def start(local_rank, args):
    ...
    if args.eval_dataset_name is not None:
        dev_list = alphaNLI.get_data(args.eval_dataset_name)
    else:
        dev_list = alphaNLI.get_data('test')
    ...
    if args.global_rank in [-1, 0]:  # only do logging if we use cpu or global_rank=0
        if not args.debug_mode:
            ...
            prediction_path = Path(file_path_prefix) / "predictions_ontest"
