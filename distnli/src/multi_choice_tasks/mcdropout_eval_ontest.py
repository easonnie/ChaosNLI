...
def start(local_rank, args):
    ...
    if args.global_rank in [-1, 0]:  # only do logging if we use cpu or global_rank=0
        if not args.debug_mode:
            ...
            prediction_path = Path(file_path_prefix) / "mtdropout_predictions_ontest"
    ...
    if args.global_rank in [-1, 0]:
        r_dict = dict()
        pred_output_list = eval_model_mtdropout(model, dev_dataloader, args.global_rank, args, T=args.mcdropout_pass_num)
