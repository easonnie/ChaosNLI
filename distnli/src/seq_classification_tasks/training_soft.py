...
def main():
    ...
    parser.add_argument("--train_dataset_name", type=str, help='the name tag for the training dataset')
    parser.add_argument("--dev_dataset_name", default=None, type=str, help='the name tag for the training dataset')
    parser.add_argument(
        "--load_model_path",
        default=None,
        type=str,
...
def train(local_rank, args):
    ...
    if args.dev_dataset_name is not None:
        test_snli_list = nli.get_data('snli', args.dev_dataset_name)
        mnli_dev_mm_list = nli.get_data('mnli', args.dev_dataset_name)
    else:
    ...
    for epoch in tqdm(range(num_epoch), desc="Epoch", disable=args.global_rank not in [-1, 0]):
        ...
        for forward_step, batch in enumerate(tqdm(train_dataloader, desc="Iteration",
            ...
            logits = outputs[0]
            loss = softCrossEntropy(logits.view(-1, number_of_classification_label), batch['y'].view(-1, number_of_classification_label))
